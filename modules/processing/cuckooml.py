...
def init_cuckooml():
    ...
    data_directory = CUCKOO_ROOT + '/' + cfg.cuckooml.data_directory
    ...
    if use_cache and os.path.exists(dump_file_path):
        ...
        del simple_features_dict, features_dict, labels_dict, loader

        if use_cache:
            with open(dump_file_path, 'wb') as f:
                pickle.dump(ml, f, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    if cfg.cuckooml.clustering:
        ...
        # print list(selected_features[0])
        # ml.draw_box_plot(dataset=selected_features[0], \
        #     columns={
        #     ':simp:section:count:uncommon': 'linear',
        #     ':pef:oph:size_image': 'log',
        #     ':simp:section:entropy:mean': 'linear',
        #     ':file:count:failed': 'linear'
        #     })
        # sys.exit(0)
        ...
        for f, d in itertools.izip(filters, selected_features):
            if f == "log_bin":
                data.append(d.applymap(__log_bin))
            elif f == "filter_dataset_dense":
                data.append(ml.filter_dataset(d, feature_coverage=0.35, complement=False))
            elif f == "filter_dataset_pca":
                data.append(ml.filter_dataset_pca(d))
            elif f == "feature_selection":
                data.append(ml.feature_selection(d, classifier=XGBClassifier, k=100))
            elif f == "none":
                data.append(d)
        ...
        if cls_alg == "hdbscan":
            ...
            ml.cluster_hdbscan(data, True, *cl_args)
        elif cls_alg == "dbscan":
            cl_args= [int(i) for i in cl_args]
...
class ML(object):
    ...
    def extract_features(self, features, include_API_calls=False, \
        ...
        # for i in features:
        #     simp_counter += Counter(features[i]["static_imports"].keys())
        #     dimp_counter += Counter(features[i]["dynamic_imports"])
        #     section_counter += Counter(features[i]["section_attrs"].keys())
        
        # significant_count = int(SIGNIFICANCE_FRACTION*len(features))
        # simp_common = zip(*simp_counter.most_common(significant_count))[0]
        # dimp_common = zip(*dimp_counter.most_common(significant_count))[0]
        ...
        for i in features:
            ...
            #             my_features[i].get(":simp:section:count:uncommon", 0) + 1
            # Section entropy statistics
            # entropies = features[i]["section_attrs"].values()
            # if entropies != None and len(entropies) > 1:
            #     my_features[i][":simp:section:entropy:min"] = min(entropies)
            #     my_features[i][":simp:section:entropy:max"] = max(entropies)
            #     my_features[i][":simp:section:entropy:mean"] = statistics.mean(entropies)
            #     my_features[i][":simp:section:entropy:var"] = statistics.variance(entropies)
            #     my_features[i][":simp:section:entropy:median"] = statistics.median(entropies)
            #     my_features[i][":simp:section:entropy:stdev"] = statistics.stdev(entropies)
            #     my_features[i][":simp:section:entropy:mode"] = max(set(entropies), key=entropies.count)

                    # float(features[i]["section_attrs"][s])
            # for r in features[i]["resource_attrs"].keys():
            #     my_features[i][":simp:resource:"+r] = 1
                    # features[i]["resource_attrs"][r]

            # Vectorize optional headers
            # for j in features[i]["optional_headers"].keys():
            #     my_features[i][":pef:oph:"+j] = features[i]["optional_headers"][j]
            ...
            for j in static_imports_dlls:
                ...
                #         my_features[i].get(":simp:count:uncommon", 0) + 1
            ...
            for di in features[i]["dynamic_imports"]:
                ...
                # if di in dimp_common:
                #     my_features[i][":dimp:" + di] = 1
                # else:
                #     my_features[i][":dimp:count:uncommon"] = \
                #         my_features[i].get(":dimp:count:uncommon", 0) + 1
            ...
            for t in touch:
                for f in features[i][t]:
                    my_features[i][":file:touch:" + f.lower()] = 1
                    # while f.find('\\') != -1:
                    #     f = f.rsplit('\\', 1)[0]
                    #     my_features[i][":file:touch:" + f.lower()] = \
                    #         my_features[i].get(":file:touch:" + f.lower(), 0) + 1
    ...
    def filter_dataset_pca(self, dataset=None, variance_coverage=None):
        ...
        if variance_coverage is None:
            return pd.DataFrame(data=PCA().fit_transform(dataset))
        ...
        # dataset =  PCA(n_components=next(i for i in range(len(variances)) if variances[i] > 0.96)).fit_transform(dataset)

    def feature_selection(self, dataset=None, labels=None, k=None, classifier=RandomForestClassifier):
        """Feature reduction using PCA"""
        if dataset is None:
            dataset = self.features.copy()
        ...
        selected_indices = model.feature_importances_.argsort()[::-1][:k]
        return pd.DataFrame(data=X[:, selected_indices], columns=dataset.columns[selected_indices])
    ...
    def draw_box_plot(self, dataset=None, labels=None, columns={}):
        ...
        for column in columns.keys():
            df = pd.DataFrame(dataset[column]).join(labels, how='inner')
            ...
            df.boxplot(by='label', sym='k.', whis=[5,95], showmeans=True)

            if columns[column] in ['linear', 'log']:
                plt.yscale(columns[column])
            plt.show()

            # Print the statistics along with the boxplot
            for l in labels.label.unique():
                print l, df[df['label'] == l].describe()  
    ...
    def cluster_hdbscan(self, features=None, do_grid_search=False,min_samples=1, min_cluster_size=6, dry=False):
        ...
        if do_grid_search:
            ...
            param_grid = {'min_cluster_size': range(min_samples+1, int(features.shape[0]**(1.0/2)))}
            # param_grid = {'min_samples': range(min_samples, min_cluster_size)}
            best_score = 0
            
            for g in ParameterGrid(param_grid):
                hdbscan.set_params(**g)
                hdbscan_fit = hdbscan.fit(features)
                print g,

                merged_labels = [hdbscan_fit.labels_, labels]
                ari_score = metrics.adjusted_rand_score(*merged_labels)
                ami_score = metrics.adjusted_mutual_info_score(*merged_labels)
                homogeneity = metrics.homogeneity_score(*merged_labels)
                completeness = metrics.completeness_score(*merged_labels)
                v_score = metrics.v_measure_score(*merged_labels)
                print "clustering+noise", ami_score, ari_score, homogeneity, completeness, v_score

                # merged_labels = zip(*merged_labels)
                # merged_labels = [x for x in merged_labels if x[0] != -1]
                # merged_labels = [list(t) for t in zip(*merged_labels)]

                # v_score = metrics.v_measure_score(*merged_labels)
                # homogeneity = metrics.homogeneity_score(*merged_labels)
                # completeness = metrics.completeness_score(*merged_labels)
                # print "clustering", v_score, homogeneity, completeness
    ...
    def load_binaries(self, directory):
        ...
        for f in os.listdir(directory):
            ...
            try:
                ...
                self.binaries[f].label_sample(label_type='binary')
                self.binaries[f].extract_features()
                self.binaries[f].extract_basic_features()

                # TEMP Deleting instance reports
                # This is to save excessive use of memory
                # Instance objects couldn't be used to update & save json
                del self.binaries[f].report
            except AttributeError as e:
                del self.binaries[f]
                print >> sys.stderr, "Error in line {}".format(sys.exc_info()[-1].tb_lineno)
                print >> sys.stderr, e
...
class Instance(object):
    ...
    def label_sample(self, external_labels=None, label_type="family"):
        ...
        if label_type == 'binary':
            if self.positives is None or self.positives < self.LABEL_SIGNIFICANCE_COUNT:
                self.label = 'benign'
            else:
                self.label = 'malware'
            return
        ...
        if external_labels is None and self.scans is not None:
            for vendor in self.scans:
                merged_labels += self.scans[vendor]["normalized"]
    ...
    def feature_static_pef(self):
        ...
        # Optional headers
        # self.features["optional_headers"] = self.report.get("static", {}).get('optional_headers', {})
